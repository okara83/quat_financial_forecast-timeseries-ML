...
forward = 2 # choose how many steps in the future you want to see
input_dim = 1
hidden_dim = 128
num_layers = 2
output_dim = forward
...
########################################################################################################################
########################################################################################################################
### Functions for multistep prediction and sliding window
########################################################################################################################
########################################################################################################################


# Bug if forward =1, use the function in RNN_1
def split_data(stock, lookback,forward):
    ...
    y_train = data[forward-1:train_set_size+forward-1,-forward:,:]
    
    x_test = data[train_set_size:,:-1]
    x_test = x_test[:-forward+1,:,:]
    y_test = data[train_set_size+ forward-1:,-forward:,:]
...
y_train= torch.from_numpy(y_train).type(torch.Tensor).squeeze()
y_test = torch.from_numpy(y_test).type(torch.Tensor).squeeze()
...
scale = MinMaxScaler()
scale.min_, scale.scale_ = minmax.min_, minmax.scale_
